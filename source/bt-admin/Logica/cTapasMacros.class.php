<?php  
include(DIR_CLASES_DB."cTapasMacros.db.php");

class cTapasMacros extends cTapasMacrosdb	
{
	protected $conexion;
	protected $formato;
...
//		la función retorna true o false si se pudo ejecutar con éxito o no
	...
	{
		$oTapasMacrosColumnas = new cTapasMacrosColumnas($this->conexion);
		if(!$oTapasMacrosColumnas->BusquedaAvanzada ($datos,$resultado,$numfilas))
			return false;
		if ($numfilas > 0)
		{
			if($muestromsg)
				FuncionesPHPLocal::MostrarMensaje($this->conexion,MSG_ERRGRAVE,"Error, la macro tiene columnas asignadas.",array("archivo" => __FILE__,"funcion" => __FUNCTION__, "linea" => __LINE__),array("formato"=>""));
			return false;
		}	
		return true;
