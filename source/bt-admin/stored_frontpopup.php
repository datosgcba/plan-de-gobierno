...
$result=true;
if (!$oStoredFront->TraerTablas(BASEDATOS,$query))
	$result=false;
...
while ($tabla = $conexion->ObtenerSiguienteRegistro($query)) 
{
 ?>
	<tr>	
		...
		<td width="80%" >
			<input type="radio" id="opc_<? echo $tabla[$Tables]?>" name="tablas" value="<? echo $tabla[$Tables]?>" <? if ($primero) echo "checked"?> > <label for="opc_<? echo $tabla[$Tables]?>"><? echo $tabla[$Tables];?> </label>
		</td>
	</tr>	
 <?
 $primero=false;
...
}else
{
	...
	if ($result)
	{
		if (!$oStoredFront->TraerCampos($tablaencontrada,$resultado))
			$result=false;
	...
	{?>
		...
		<div align="right" style="width:97%">
			<a href="stored_frontpopup.php" class="linkfondoblanco textoaclaraciones">Volver</a>
		</div>
		<table width="100%" class="textotabla">
			<tr>
			...
			<tr>
				<td valign="top" width="100%">
				<textarea cols="50" rows="7" name="Texto"><? echo $textoescribir?></textarea>
				</td>
			</tr>
			<tr>	
				<td width="100%" align="left">
					  <input type="button" name="boton" class="botones" value="Agregar"  onclick="SetearTexto('spsqlstring',formulario_popup.Texto.value,window.opener.document);SetearTexto('sptabla','<? echo $NombreTabla?>',window.opener.document); SetearTexto('spoperacion','<? echo $OperacionMostrar; ?>',window.opener.document); SetearTexto('spnombre','<? echo $NombreStored; ?>',window.opener.document);window.close();">
				</td>
			</tr>	
		</table>
		</form>	
