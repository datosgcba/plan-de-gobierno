...
$copete = $datosnoticia['noticiacopete'];
...
if (isset($objDataModel->Texto) && $objDataModel->Texto!="")
	$copete =  utf8_decode($objDataModel->Texto);
...
{	
	...
	if ($numfilas!=0)
	{
		...
		switch($datosVideo['multimediatipocod'])
		{
			...
			break;	
			case YOU:
				$urlVideo = '<iframe class="MultimediaVideo" src="http://www.youtube.com/embed/'.$datosVideo['multimediaidexterno'].'?wmode=transparent&showinfo=0&autohide=1" frameborder="0" allowfullscreen wmode="transparent"></iframe>';	
			break;	
		}
		$conFoto = false;
	}
...
?>
<div class="nota <?php  echo $clase?> tap_modules<?php  if ($BordeInferior) echo " borde_inferior"?> clearfix" id="module_<?php  echo $vars['zonamodulocod']?>" <?php  echo  $vars['mouseaction']?>>
...
<?php  switch($noticiaTipo){
	case 1: // noticia siempre?>	
        ...
        <?php  }?>    
        <?php  if ($conCategoria || $conVolanta || $conHora /*|| $conPais*/){?>
	...
	case 3: 
		...
		if ($conFoto){
			 $altoImg=ceil($anchoImg*0.66);
			 $imgurl=DOMINIO_SERVIDOR_MULTIMEDIA.$oMultimedia->GetImagenStatic($anchoImg, $altoImg, $datosFoto["multimediacatcarpeta"]."N/".$datosFoto["multimediaubic"],true,1);
			?>
            <a href="<?php  echo  $dominio?>" target="<?php  echo $target?>" title="<?php  echo  FuncionesPHPLocal::HtmlspecialcharsBigtree($titulo,ENT_QUOTES)?>">
                <figure>
					<img class="img-responsive" src="<?php  echo $imgurl?>" alt="Imagen de <?php  echo  FuncionesPHPLocal::HtmlspecialcharsBigtree($titulo,ENT_QUOTES)?>"/>
                   	<figcaption>
	...
	<?php  case 4: //notaSecundariaFotoPrimero?>	               
        ...
        <?php  }?>          
        <?php  if ($conVinculadas){?>
        	<div class="caja_noticias_relacionadas">
