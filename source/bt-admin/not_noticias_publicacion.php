...
$noticiaduplicada = false;
if ($datosnoticia['noticiacopiacod']!="")
	$noticiaduplicada = true;
...
if (!$noticiaduplicada)
{
	...
	$datos['noticiaestadocod'] = $noticiaestadocod;
	$datos['arregloestadofinal'][] = NOTDESPUBLICADAS;
	if(!$oNoticiasAcciones->ObtenerAccionesRol($datos,$resultadoacciones,$numfilasacciones))
		return false;	
	
	$accioneliminar = false;
	if($oNoticiasAcciones->TieneAccionEliminar($datos,$nombrebotoneliminar,$noticiaworkflowcodpub))
	{	
		FuncionesPHPLocal::ArmarLinkMD5("not_noticias_eliminar.php",array("noticiacod"=>$noticiacod,"noticiaworkflowcod"=>$noticiaworkflowcodpub,"accion"=>1),$geteliminar,$md5eliminar);
		$accioneliminar = true;
	}
	$acciondespublicar = false;
	if($oNoticiasAcciones->TieneAccionDespublicar($datos,$nombrebotondespublicar,$noticiaworkflowcoddespub))
	{	
		FuncionesPHPLocal::ArmarLinkMD5("not_noticias_despublicar.php",array("noticiacod"=>$noticiacod,"noticiaworkflowcod"=>$noticiaworkflowcoddespub,"accion"=>1),$getdespublicar,$md5despublicar);
...
<link href="modulos/not_noticias/css/noticias_publicadas.css" rel="stylesheet" title="style" media="all" />
...
<div class="row">
    <span class="obligatorio">(*)</span> Recuerde <span style="font-weight:bold;">guardar</span>  antes de <span style="font-weight:bold;">publicar</span> una noticia para salvar los cambios realizados.
</div>
...
<div id="DetalleNoticiaAm">
        ...
        <ul class="accionesnoticia">
			...
			if (!$noticiaduplicada)
			{
                ...
                while ($fila = $conexion->ObtenerSiguienteRegistro($resultadoacciones)){
					...
					?>
                        <li> <a class="btn btn-default" href="not_noticias_bajar_publicacion.php?<?php  echo $getbajar?>"  class="<?php  echo $class?>" id="<?php  echo $fila['noticiaestadocodfinal']?>" rel="<?php  echo $fila['noticiaworkflowcod']?>" ><?php  echo  FuncionesPHPLocal::HtmlspecialcharsBigtree($fila['noticiaaccion'],ENT_QUOTES);?></a></li>
                        <?php 
                    $i++;
                    $class = "middle";
                }
				 if ($acciondespublicar){?>
					<li ><a class="btn btn-danger" href="not_noticias_despublicar.php?<?php  echo $getdespublicar?>" onclick="if (!confirm('Esta seguro que desea despublicar la noticia?')) return false;"><?php  echo  FuncionesPHPLocal::HtmlspecialcharsBigtree($nombrebotondespublicar,ENT_QUOTES);?></a></li>
				<?php  }?>
				<?php  if ($accioneliminar){?>
					<li ><a class="btn btn-danger" href="not_noticias_eliminar.php?<?php  echo $geteliminar?>" onclick="if (!confirm('Esta seguro que desea eliminar la noticia?')) return false;"><?php  echo  FuncionesPHPLocal::HtmlspecialcharsBigtree($nombrebotoneliminar,ENT_QUOTES);?></a></li>
				<?php  }?>
            <?php  }?>
    ...
    <div class="clear fixalto">&nbsp;</div>
            ...
            <div class="clearboth brisa_vertical">&nbsp;</div>
            <div class="aire">
            	<?php  if ($noticiacomentarios==1) {echo 'SI';} else {echo "NO";}?>
            </div>
            <div class="clearboth brisa_vertical">&nbsp;</div>
            ...
            <div class="clearboth brisa_vertical">&nbsp;</div>
            <div class="aire">
            	<?php  if ($noticiadestacada==1) {echo 'SI';} else {echo "NO";}?>
            </div>
            <div class="clearboth brisa_vertical">&nbsp;</div>
            ...
            <ul class="accionesnoticia">
                ...
                if (!$noticiaduplicada)
                {
                    ...
                    while ($fila = $conexion->ObtenerSiguienteRegistro($resultadoacciones)){
                     ...
                     if ($accioneliminar){?>
                        <li><a class="btn btn-danger"  href="not_noticias_eliminar.php?<?php  echo $geteliminar?>" onclick="if (!confirm('Esta seguro que desea eliminar la publicacion?')) return false;"><?php  echo  FuncionesPHPLocal::HtmlspecialcharsBigtree($nombrebotoneliminar,ENT_QUOTES);?></a></li>
                    <?php  }?>
                <?php  }?>
                <li><a  class="btn btn-default" href="not_noticias.php">Volver</a></li>
            </ul>
