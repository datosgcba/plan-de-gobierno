...
$datos['formatocod'] = $_GET['formatocod'];
if(!$oMultimediaFormatos->BuscarMultimadiaFormatoxCodigo($datos,$resultadoFormato,$numfilas))
	return false;

$filaFormato = $conexion->ObtenerSiguienteRegistro($resultadoFormato);

$carpetaFecha = date("Ym")."/";
$carpetadestino = CARPETA_SERVIDOR_MULTIMEDIA_FISICA."noticias/".$filaFormato['formatocarpeta']."/";
...
while ($fila = $conexion->ObtenerSiguienteRegistro($resultadoNoticias))
{
	...
	if (file_exists($carpetalocal.$fila["multimediaubic"]) && (!file_exists($carpetadestino.$fila["multimediaubic"]) || $pisaExistentes==true))
	{
		...
		{
			if(!$oImagen->CropearImagen($nombrearchivo,$carpetalocal,$carpetadestino,$filaFormato['formatoancho'],$filaFormato['formatoalto']))
			{
				echo "error1";
			}
		}/*else
