...
function doSearch(ev){ 
	...
	jQuery("#listarDatos").jqGrid('setGridParam', {url:"plan_proyectos_estados_lst_ajax.php?rand="+Math.random(), postData: datos,page:1}).trigger("reloadGrid");
}
function Resetear(){
	$("#planproyectoestadocod").val("");
	$("#planproyectoestadonombre").val("");
	timeoutHnd = setTimeout(gridReload,500);
}
	...
	{
		url:'plan_proyectos_estados_lst_ajax.php?rand='+Math.random(),
		postData: datos,
		datatype: "json", 
		colNames:['C\u00f3digo','Nombre','Estado','Editar','Eliminar'],
		colModel:[
			{name:'planproyectoestadocod',index:'planproyectoestadocod', width:25, align:'center'},
			{name:'planproyectoestadonombre',index:'planproyectoestadonombre', align:'left'},
		...
		mtype: "POST",
		pager: '#pager2',
		sortname: 'planproyectoestadocod',
		viewrecords: true,
		sortorder: "DESC", 
		height:290, 
		caption:"", 
	...
	param = "planproyectoestadocod="+codigo;
	param += "&accion="+tipo;
	EnviarDatos(param);
	...
	$.ajax({
		type: "POST",
		url: "plan_proyectos_estados_upd.php",
		data: param,
		dataType:"json",
