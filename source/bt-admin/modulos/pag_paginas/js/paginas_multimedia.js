...
function AbrirPopupMultimedia(titulo,archivo)
{
	...
	if ($("#pagcod").val()=="")
	{
		alert("Debe guardar la pagina antes de subir un archivo multimedia");
		return false;			
	}
...
function GuardarImagen()
{
	...
	$.ajax({
	   type: "POST",
	   url: "pag_paginas_multimedia_upd.php",
	   data: param,
	   dataType:"json",
	   success: function(msg){ 
			if (msg.IsSucceed)
...
{
	if (!confirm("Esta seguro que desea eliminar el archivo multimedia de la pagina?"))
		return false;
...
function CargarMultimedia(tipo,idCarga)
{
	...
	$.ajax({
	   type: "POST",
	   url: "pag_paginas_multimedia_lst.php",
	   data: param,
	   dataType:"html",
	   success: function(msg){ 
			...
			$(tipo).sortable(
			  { 
				...
				update: function() {
					...
					var order = $(this).sortable("serialize")+"&pagcod="+$("#pagcod").val()+"&accion=5"; 	
					$("#MsgGuardando").show();
					$.post("pag_paginas_multimedia_upd.php", order, function(msg){
						if (msg.IsSucceed)
						{
							$(".msgaccionpagina").html("&nbsp;");
						}else
