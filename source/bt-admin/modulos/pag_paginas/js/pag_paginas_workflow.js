...
function gridReload(){ 
	...
	jQuery("#ListarWorkflow").jqGrid('setGridParam', {url:"pag_paginas_workflow_lst_ajax.php?rand="+Math.random(), postData: datos,page:1}).trigger("reloadGrid"); 
} 
...
function Resetear()
{
	$("#paginaestadocodinicial").val("");
	$("#paginaestadocodfinal").val("");
	timeoutHnd = setTimeout(gridReload,500) 
}
...
function ListarWorkflow()
{
	...
	{ 
				url:'pag_paginas_workflow_lst_ajax.php?rand='+Math.random(),
				postData: datos,
				datatype: "json", 
				colNames:['COD','Estado Inicial','Estado Final','Accion','Editar','Eliminar'], 
				colModel:[ {name:'paginaworkflowcod',index:'paginaworkflowcod', width:10, align:"center", hidden:true}, 
						  {name:'paginaestadocodinicial',index:'paginaestadocodinicial'},
						  {name:'paginaestadocodfinal',index:'paginaestadocodfinal'},
						  {name:'paginaaccion',index:'paginaaccion'}, 
...
function FormWorkflow(modif,paginaworkflowcod)
{
	...
	$.ajax({
	   type: "POST",
	   url: "pag_paginas_workflow_am.php",
	   data: param,
	   success: function(msg){
			$("#Popup").dialog({	
				height: 350, 
...
function EditarWorkflow(paginaworkflowcod)
{
	FormWorkflow(1,paginaworkflowcod);
	return true;
}
...
function EnviarDatos(param)
{
	...
	$.ajax({
	   type: "POST",
	   url: "pag_paginas_workflow_upd.php",
	   data: param,
	   dataType:"json",
	   success: function(msg){
...
function EliminarWorkflow(paginaworkflowcod)
{
