...
function doSearch(ev){ 
	...
	jQuery("#listarDatos").jqGrid('setGridParam', {url:"plan_jurisdicciones_lst_ajax.php?rand="+Math.random(), postData: datos,page:1}).trigger("reloadGrid");
}
function Resetear(){
	$("#planjurisdiccioncod").val("");
	$("#planjurisdiccionnombre").val("");
	timeoutHnd = setTimeout(gridReload,500);
}
	...
	{
		url:'plan_jurisdicciones_lst_ajax.php?rand='+Math.random(),
		postData: datos,
		datatype: "json", 
		colNames:['C\u00f3digo','Nombre','Estado','Editar','Eliminar'],
		colModel:[
			{name:'planjurisdiccioncod',index:'planjurisdiccioncod', width:25, align:'center'},
			{name:'planjurisdiccionnombre',index:'planjurisdiccionnombre', align:'left'},
	...
	param = "planjurisdiccioncod="+codigo;
	param += "&accion="+tipo;
	EnviarDatos(param);
	...
	$.ajax({
		type: "POST",
		url: "plan_jurisdicciones_upd.php",
		data: param,
		dataType:"json",
