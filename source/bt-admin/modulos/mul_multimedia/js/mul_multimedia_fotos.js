...
jQuery(document).ready(function(){
	$( "#mul_multimedia_fotos" ).tabs({
		beforeActivate: function( event, ui ) {
			if (ui.newPanel.attr('id')=="mul_multimedia_seleccionar_imagen")
	...
	if ($('#mult_carga_multimedia_imagenes').val()==1){
...
function CrearBtUploadImgMultiple(){
	  ...
	  var uploader = new qq.FileUploader({
          element: document.getElementById("mul_multimedia_bt_subir_mult_img"),
          action: "mul_multimedia_carga_temporal_multiples.php",
		  method: "POST",
          multiple: true,
          ...
          template: '<div class="qq-uploader">' +
                        ...
                        '<div class="qq-upload-button boton verde">Subir varias Imagenes</div>' +
		 ...
		 onComplete: function(id, fileName, responseJSON) {
				 ...
				 if (responseJSON.success==true){
				 	...
				 	alert("Fallo en la subida del archivo");	
          },
          messages: {
          },
...
function CrearBtUploadImagenes(){
	 ...
	 var inputFile = argv[5];
	  ...
	  var uploader = new qq.FileUploader({
          ...
          template: '<div class="qq-uploader">' +
                        '<div class="qq-upload-drop-area" style="display: none;"><span>Mover aquí para subir</span></div>' +
                        '<div class="qq-upload-button boton verde" >Subir una Imgen</div>' +
                        '<div class="qq-upload-size"></div>' +
                        '<ul class="qq-upload-list"></ul>' +                        
                    '</div>',   
