...
function gridReload(){
	...
	jQuery("#listarDatos").jqGrid('setGridParam', {url:"not_comentarios_lst_ajax.php?rand="+Math.random(), postData: datos,page:1}).trigger("reloadGrid");
}
function Resetear(){
	$("#comentarionombre").val("");
	$("#comentarioemail").val("");
	timeoutHnd = setTimeout(gridReload,500);
}
...
function listar(){
	...
	{
		url:'not_comentarios_lst_ajax.php?rand='+Math.random(),
		postData: datos,
		datatype: "json", 
		colNames:['Codigo','Nombre','Email','Titulo noticia','Fecha Alta','Estado','Editar','Eliminar'],
		colModel:[
			{name:'comentariocod',index:'comentariocod', width:25, align:'center'},
			{name:'comentarionombre',index:'comentarionombre',width:80, align:'left'},
			{name:'comentarioemail',index:'comentarioemail',width:140, align:'left'},
			{name:'noticiatitulo',index:'noticiatitulo', align:'left'},
			{name:'comentariofalta',index:'comentariofalta', width:40, align:'center'},
