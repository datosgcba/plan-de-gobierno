...
function doSearch(ev){ 
	...
	jQuery("#listarDatos").jqGrid('setGridParam', {url:"fot_fotos_dia_lst_ajax.php?rand="+Math.random(), postData: datos,page:1}).trigger("reloadGrid");
}
function Resetear(){
	$("#fotodiacod").val("");
	$("#fotodiatitulo").val("");
	...
	{
		url:'fot_fotos_dia_lst_ajax.php?rand='+Math.random(),
		postData: datos,
		datatype: "json", 
		colNames:['Codigo','Titulo','Fecha','Estado','Editar','Eliminar'],
		colModel:[
			{name:'fotodiacod',index:'fotodiacod', width:25, align:'center'},
			{name:'fotodiatitulo',index:'fotodiatitulo', align:'left'},
			{name:'fotofecha',index:'fotofecha', align:'left'},
