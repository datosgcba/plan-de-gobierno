...
function doSearch(ev){ 
	...
	jQuery("#listarDatos").jqGrid('setGridParam', {url:"plan_tags_categorias_lst_ajax.php?rand="+Math.random(), postData: datos,page:1}).trigger("reloadGrid");
}
function Resetear(){
	$("#plantagcatcod").val("");
	$("#plantagcatnombre").val("");
	timeoutHnd = setTimeout(gridReload,500);
}
	...
	{
		url:'plan_tags_categorias_lst_ajax.php?rand='+Math.random(),
		postData: datos,
		datatype: "json", 
		colNames:['C\u00f3digo','Nombre','Estado','Editar','Eliminar'],
		colModel:[
			{name:'plantagcatcod',index:'plantagcatcod', width:25, align:'center'},
			{name:'plantagcatnombre',index:'plantagcatnombre', align:'left'},
	...
	$.ajax({
		type: "POST",
		url: "plan_tags_categorias_upd.php",
		data: param,
		dataType:"json",
