...
//----------------------------------------------------------------------------------------- 
function Validarphp($conexion,$oStored)
{
...
$result=true;
	if (!Validarphp($conexion,$oStored))
	{
	...
	if (isset ($_POST['botonbuscar']))
	{
		...
		if (!$oStored->VerificarExistenciaStore ($spnombre,$carpetas,$encontrado,$texto))
			$result=false;
	...
	if ($result &&  ((isset($_POST['botonalta']) || isset($_POST['botonbaja']))))
	{
		...
		if($result && isset($_POST['botonbaja'])) 
		{
				if ($result)
				{
					...
					if ($result)
					{
						if (!$oStored->Eliminar ($ArregloDatos))
							$result=false;
			...
			if($_POST['spcodviejo']=="") 
			{
				...
				if ($result)
				{
					...
					if ($result)
					{
						...
						if (!$oStored->Insertar ($ArregloDatos,$codigosp))
							$result=false;
			...
			{	
				...
				if ($result)
				{
					...
					if ($result)
					{
						...
						if (!$oStored->Buscar ($Arreglonombre,$numfilas,$resultadonombre))
							$result=false;
						...
						if ($result)
						{
							...
							$ArregloDatos['spsqlstring']=$_POST['spsqlstring'];	
							$ArregloDatos['spobserv']=$_POST['spobserv'];	
							if (!$oStored->Modificar ($ArregloDatos))
								$result=false;
						}
						if ($result)
						{
...
?>
			<br /><br /><br /><div align="center"><a href="stored.php" class="linkfondoblanco">Volver</a></div>
    </div>
</div>        
