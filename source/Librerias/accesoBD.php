...
class accesoBDLocal
{
	...
	{
		$this->idconexion = mysql_connect($servidor, $usuariodb, $clave, true) or die("No es posible establecer conexion con MySQL Server");
	}
//-----------------------------------------------------------------------------------------
//							 PUBLICAS	
//----------------------------------------------------------------------------------------- 
...
// Permite la selección de la base de datos sobre la cual se va a trabajar.
	...
	{
		mysql_select_db($base_datos,$this->idconexion) or die("No es posible conectar a la base de datos");
	...
	function IraPosicionResultado(&$resultado,$posicion=0)
	{
		mysql_data_seek($resultado,$posicion);
	}
...
//			-> en caso contrario, retorna false y el numero de error
	...
	function ejecutarStoredProcedure($sp_nombre,$sp_param,&$resultadosalida,&$numfilassalida,&$errnosalida)
	{
		...
		{ 
			...
			FuncionesPHPLocal::MostrarMensaje($this,"Problema en la llamada al stored procedure, no se envia un array. ",array("archivo" => __FILE__,"funcion" => __FUNCTION__, "linea" => __LINE__));
			return false;
		}	

		$sql ="SELECT * FROM stored_procedures_front";
		$sql.=" WHERE ucase(spnombre)='".strtoupper($sp_nombre)."'";
		
		if(!$this->_EjecutarQuery($sql,'ejecutarStoredProcedure',$resultado,$errno))
		{
			FuncionesPHPLocal::MostrarMensaje($this,"Problema en la tabla stored_procedures. ",array("archivo" => __FILE__,"funcion" => __FUNCTION__, "linea" => __LINE__));
			return false;
		}
		...
		{
			FuncionesPHPLocal::MostrarMensaje($this,"No se encuentra el procedimiento: ".$sp_nombre." en la tabla stored_procedures. ",array("archivo" => __FILE__,"funcion" => __FUNCTION__, "linea" => __LINE__));
			return false;
		} 
		...
		{
			FuncionesPHPLocal::MostrarMensaje($this,"Error en los parámetros enviados a ".$sp_nombre.". ",array("archivo" => __FILE__,"funcion" => __FUNCTION__, "linea" => __LINE__));
			return false;
		}
...
// echo "<br /> ANTES ".$fila['spsqlstring'];		
		...
		{
			FuncionesPHPLocal::MostrarMensaje($this,"Error al reemplazar parámetros en ".$sp_nombre.". ",array("archivo" => __FILE__,"funcion" => __FUNCTION__, "linea" => __LINE__));
			return false;
		}
		...
		{
			FuncionesPHPLocal::MostrarMensaje($this,"Problema en la ejecución del stored procedure. ",array("archivo" => __FILE__,"funcion" => __FUNCTION__, "linea" => __LINE__));
			return false;
		}
		
		if($fila['spoperacion']=="SEL") 
			$numfilassalida=mysql_num_rows($resultadosalida);
		else
			$numfilassalida=mysql_affected_rows($this->idconexion);
	...
	function ManejoTransacciones($transtipo)
	{
		switch($transtipo)
		{
			...
			case "R": // rollback
				...
				break;
			default:
				FuncionesPHPLocal::MostrarMensaje($this,"Tipo de transacción no definida",array("archivo" => __FILE__,"funcion" => __FUNCTION__, "linea" => __LINE__));
				return false;
				break;
		}
		...
		{ 
			...
			for ($i=0;$i<count($arraywhere);$i++)
			{
				if ($i%2 == 0)
					...
					$sql.=mysql_real_escape_string($arraywhere[$i],$this->idconexion);
			}
		}
	
		if (!$this->_EjecutarQuery($sql,'TraerCampo',$resultado,$errno)) 
		{
			FuncionesPHPLocal::MostrarMensaje($this,"Problema en TraerCampo.",array("archivo" => __FILE__,"funcion" => __FUNCTION__, "linea" => __LINE__));
			return false;
		}
...
//		Si se ejecutó con exito, retorna true y el numero de filas afectadas
	...
	function ActualizarCampo($tabla,$camponom,$campovalor,$arraywhere,$datosauditoria,&$numfilas,&$errno)
	{
		...
		if($campovalor=="NULL")
			...
			$sql.=" set $camponom='".mysql_real_escape_string($campovalor,$this->idconexion)."'";
		if ($datosauditoria=="S")
		{
		...
		{
			FuncionesPHPLocal::MostrarMensaje($this,"Problema en ActualizarCampo.",array("archivo" => __FILE__,"funcion" => __FUNCTION__, "linea" => __LINE__));
			return false;
		}
		
		$numfilas=mysql_affected_rows($this->idconexion);
		return true;
	}
...
// Retorna el texto del último error producido
	...
	function TextoError()
	{
		return mysql_error($this->idconexion);
	}
...
// Retorna el ultimo codigo insertado
	...
	{
		return mysql_insert_id($this->idconexion);
	}
...
// Obtiene la cantidad de registros de un resultado
	function EscapeString(&$campovalor)
	{
		$campovalor = mysql_real_escape_string($campovalor,$this->idconexion);	
		return $campovalor;
	}
...
//		Si se ejecutó con éxito, retorna true y el resultado del query
	...
	function _EjecutarQuery($sql,$erroren,&$resultado,&$errno)
	{
		...
		$errno=0;
		$resultado=mysql_query($sql,$this->idconexion);
		
		if(!$resultado)
		{
			$errno=mysql_errno($this->idconexion);
			$textoerror="Error Mysql: ".mysql_errno($this->idconexion)." - ".mysql_error($this->idconexion);
...
// Retorna: true o false según si están correctos los parámetros o no
	...
	function _ValidarParametros($sql,$param) 	{
		...
		if ( count($diff_array1)!=0  || count($diff_array2)!=0 ) 
		{
			...
			FuncionesPHPLocal::MostrarMensaje($this,$texto,array("archivo" => __FILE__,"funcion" => __FUNCTION__, "linea" => __LINE__));
			...
			foreach ($sql_pieces as $xsql_pieces) 
			{
				...
				if ($posicion) 
				{
					...
					if(is_array($param[$param_search]))
					{
						...
						for ($i=0;$i<count($param[$param_search]);$i++)
						{
							if ($i%2 == 0) {
								 ...
								 $sql_pieces[$posicion].=mysql_real_escape_string($param[$param_search][$i],$this->idconexion);
								 $sql_pieces_may_y_min[$posicion].=mysql_real_escape_string($param[$param_search][$i],$this->idconexion);
							}	
						}
					}
					elseif(trim($param[$param_search])=="NULL")
					{
						...
						$sql_pieces[$posicion]=mysql_real_escape_string($param[$param_search],$this->idconexion); 
						$sql_pieces_may_y_min[$posicion]=mysql_real_escape_string($param[$param_search],$this->idconexion); 
					}	
