...
// Obtiene la cantidad de registros de un resultado
	...
	{
		$campovalor = mysqli_real_escape_string($this->idconexion,$campovalor);	
		return $campovalor;
	}
...
// echo "<br /> ANTES ".$fila['spsqlstring'];		
		...
		if ($sp_nombre=="sel_exp_expedientes_documentos_botones_docaccioncod")
		{
			//echo $sql_sp;
			//die();
		}
		...
		if (!$this->_EjecutarQuery($sql_sp,"stored procedure ".$fila['spcod']." - ".$fila['spnombre'],$resultadosalida,$errnosalida))
		{
				...
				FuncionesPHPLocal::MostrarMensaje($this,"Problema en la ejecución del stored procedure ".$sp_nombre.". ",array("archivo" => __FILE__,"funcion" => __FUNCTION__, "linea" => __LINE__));
	...
	function BuscarRegistroxClave($sp_nombre,$param,$arraybusq,&$resultado,&$filaret,&$numfilasmatcheo,&$errno)
	{
		...
		while($fila=$this->ObtenerSiguienteRegistro($resultado))
		{
			foreach ($arraybusq as $clave => $valor)
			{
				...
				{
					FuncionesPHPLocal::MostrarMensaje($this,"Problema en BuscarRegistroxClave. ",array("archivo" => __FILE__,"funcion" => __FUNCTION__, "linea" => __LINE__));
					return false;
				}
...
// Retorna: true o false según si están correctos los parámetros o no
	...
	function _ValidarParametros($sql,$param) 	{
		...
		{
				FuncionesPHPLocal::MostrarMensaje($this,"Error en la cantidad de parámetros en la llamada al procedimiento. ",array("archivo" => __FILE__,"funcion" => __FUNCTION__, "linea" => __LINE__));
